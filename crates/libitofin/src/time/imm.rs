//! IMM (International Money Market) date functions.
//!
//! Port of `ql/time/imm.{hpp,cpp}`. QuantLib resolves null-date defaults
...
const MAIN_CYCLE_CODES: &[u8] = b"HMZU";

/// Whether the given date is an IMM date (the third Wednesday of the month;
...
/// Whether the given string is an IMM code (a month letter followed by a
/// year digit, e.g. `H3`), restricted to the main cycle when `main_cycle`
/// is set.
pub fn is_imm_code(code: &str, main_cycle: bool) -> bool {
...
/// The IMM code for the given date (e.g. `H3` for March 20th, 2013).
///
/// Fails if the input date is not an IMM date.
pub fn code(imm_date: Date) -> QlResult<String> {
    crate::require!(
        is_imm_date(imm_date, false),
        "{imm_date} is not an IMM date"
    );
    let month_code = ALL_MONTH_CODES[(imm_date.month().ordinal() - 1) as usize] as char;
    Ok(format!("{month_code}{}", imm_date.year() % 10))
}
...
/// The next IMM date strictly following the given date.
pub fn next_date(date: Date, main_cycle: bool) -> Date {
    ...
    if skip_months != offset || date.day_of_month() > 21 {
        skip_months += m;
        if skip_months <= 12 {
            m = skip_months;
        } else {
    ...
    let result = Date::nth_weekday(3, Weekday::Wednesday, Month::from_ordinal(m), y);
    if result <= date {
        next_date(Date::new(22, Month::from_ordinal(m), y), main_cycle)
    } else {
        result
    }
}

/// The next IMM date strictly following the date the given IMM code maps to
/// at `reference_date`.
pub fn next_date_from_code(
    ...
    let imm_date = date(imm_code, reference_date)?;
    Ok(next_date(imm_date + 1, main_cycle))
}

/// The IMM code of the next IMM date strictly following the given date.
pub fn next_code(date: Date, main_cycle: bool) -> QlResult<String> {
    code(next_date(date, main_cycle))
}

/// The IMM code of the next IMM date strictly following the date the given
/// IMM code maps to at `reference_date`.
pub fn next_code_from_code(
    imm_code: &str,
    main_cycle: bool,
    reference_date: Date,
) -> QlResult<String> {
    code(next_date_from_code(imm_code, main_cycle, reference_date)?)
}
...
mod tests {
    ...
    #[test]
    fn imm_dates_from_2000_to_2040() {
        let last = Date::new(1, Month::January, 2040);
        let mut counter = Date::new(1, Month::January, 2000);

        while counter <= last {
            let imm = next_date(counter, false);

            assert!(imm > counter, "{imm} is not greater than {counter}",);
            assert!(
                is_imm_date(imm, false),
                "{imm} is not an IMM date (calculated from {counter})",
            );
            assert!(
                imm <= next_date(counter, true),
                "{imm} is not less than or equal to the next future in the main cycle {}",
                next_date(counter, true),
            );
            assert_eq!(
                date(&code(imm).unwrap(), counter).unwrap(),
                imm,
                "{} at calendar day {counter} is not the IMM code matching {imm}",
                code(imm).unwrap(),
            );

            for imm_code in IMM_CODES.iter().take(40) {
                assert!(
                    date(imm_code, counter).unwrap() >= counter,
                    "{} is wrong for {imm_code} at reference date {counter}",
                    date(imm_code, counter).unwrap(),
                );
            }
